...
const SIDEBAR_CONTENT = {
  ...
  trade: [
    ...
    },
    {
      ...
      body: `Each chart plots two lifecycle properties as axes, with utility scores running 0–100%. All concepts start as gray dots — that's intentional. Use the concept panel on the right to select the ones you want to focus on; selected concepts are highlighted on top of the gray dots across every chart simultaneously. The Datum is always shown at 50% on every axis (it's the baseline everything else is scored relative to). Turn the Datum on to see quadrant lines dividing the space — the upper-right green-shaded quadrant is where you want your chosen concept to land. The stepped Pareto front line marks concepts where no other option beats them on both axes at once.`
    },
    {
      title: "How to Read the Radar Chart",
      body: `The radar chart shows all selected concepts as overlapping polygons on a spider diagram. It appears automatically once your project has at least 3 lifecycle properties with Pugh scores — no configuration needed. The radar shows <em>all</em> of your scored lifecycle properties, not just the pair shown in any one scatter chart, so it gives the most complete picture of each concept's strengths and weaknesses across the full set of dimensions. The amber dashed ring at 50% on every spoke marks the datum baseline. Concepts that push outward on multiple spokes simultaneously are strong performers across dimensions — though watch for concepts that excel on some spokes at the expense of others.`
    },
    {
      title: "Tips for Getting the Most Out of Tradespace",
      body: `Start with the Pareto front — any concept on it is not beaten by any other on both axes simultaneously, making it a strong candidate. Then select those concepts in the panel and study the radar to see how they compare across all lifecycle properties at once. Use Select All to get a full-field view, then Clear All and re-select only the top candidates for a cleaner comparison. If no single concept dominates, the tradeoff dimensions you're seeing often point to requirements that deserve heavier weighting — or suggest a hybrid concept worth exploring before you converge.`
    },
  ],
